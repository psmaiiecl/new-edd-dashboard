...
export function mapChartData(data, specs) {
  ...
  const series = specs.series.map(({ name, color, key }) => {
    ...
    return { name, color, data: serieData };
  });
  return {
    series,
    total: { numeric: total, text: numberFormatter(total) },
    override: {
      xAxis: {
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    for (const col of specs.columns) {
      ...
      for (const serie of series) {
        ...
        values[label][serie.name] = { value: numberFormatter(val), raw: val };
        total += val;
      }
    ...
    rows.push({ ...parsed, values, totals, avance: avances });
  }

  return { rows, columns: specs.columns };
}
