...
import { mapPieData, mapLineData } from "../../../../../../../utils/ChartMapperFactory";
...
export function useTabGeneralPortafolio() {
    ...
    const [avanceDescargaPortafolio, setAvanceDescargaPortafolio] = useState(null);
    ...
    useEffect(() => {
        ...
        customFetch({
            ...
            // Mapear pie charts
            setDocentesValidados(mapPieData(data.docentes, mappers.entrega_informes_resultados));
            setAvancePortafolio(mapPieData(data.docentes, mappers.entrega_informes_establecimiento));
            setAvanceModuloUno(mapPieData(data.docentes, mappers.entrega_informes_sostenedor));
            setAvanceModuloDosFicha(mapPieData(data.docentes, mappers.entrega_informes_resultados));
            setAvanceModuloDosClase(mapPieData(data.docentes, mappers.entrega_informes_establecimiento));
            setAvanceModuloTres(mapPieData(data.docentes, mappers.entrega_informes_sostenedor));
            setAvanceReporteDirectores(mapPieData(data.docentes, mappers.entrega_informes_sostenedor));
            setAvanceDescargaPortafolio(mapPieData(data.docentes, mappers.entrega_informes_establecimiento));
            setAvanceVisualizacion(mapPieData(data.docentes, mappers.entrega_informes_establecimiento));
            setAvanceDescargaClase(mapPieData(data.docentes, mappers.entrega_informes_establecimiento));
            ...
            if (diario?.fechas && diario?.pfCompletado) {
                const avance_diario = mapLineData(diario, {
                    ...
                    series: [
                        { name: "Portafolios Completados", color: "#00C49F", data: "pfCompletado" },
                        { name: "Portafolios Iniciados", color: "#FFA500", data: "pfIniciados" },
                        { name: "Modulo 3 Iniciado", color: "#00BFFF", data: "m3Iniciado_cant" }
                    ],
                });
                ...
                console.warn("No se encontró información válida en 'portafolio-avance-diario'");
            }
        });
    }, []);
