...
import {
  ...
  buildGraficoCohenIA,
  buildTablaCohenIA,
  buildTablaComparacionIA,
} from "../../../utils/utils";

export function useTab(module, selectors) {
  const customFetch = useCustomFetch();
  const safeSelectors = useMemo(() => {
  ...
  const cleanFilters = useCallback(() => {
    setSelectedFilter({
      grupo: safeSelectors.length ? safeSelectors[1] : null,
      agrupacion: null,
      especialidad: null,
      modulo: module,
    });
  }, [safeSelectors, module]);
  ...
  useEffect(() => {
    ...
    setSelectedFilter((prev) => {
      ...
      return { ...prev, grupo: safeSelectors[1] };
    });
  }, [safeSelectors]);
  ...
  useEffect(() => {
    ...
    customFetch({
      route: BASE_API_URL_2025 + `/2025-cpf-ia-resultados-modulos${filters}`,
      method: "GET",
      // hasLoadPanel: false,
      shouldCache: true,
    }).then((resp) => {
      ...
      setData({
        ...
        cohen: buildGraficoCohenIA(resp.grafico_dcohen, module) || null,
        tabla_comparacion: buildTablaComparacionIA(resp.grafico_comparacion),
        tabla_cohen: buildTablaCohenIA(resp.grafico_dcohen),
      });
    });
