...
export function buildEstadoChart(setup, data, total) {
  ...
  const arrValidados = dependencias.map(
    (dep) => data[dep].validados.count || 0
  );
  const arrNoValidados = dependencias.map(
    (dep) => data[dep].no_validados.count || 0
  );
  const arrSinIngreso = dependencias.map(
    (dep) => data[dep].sin_ingreso.count || 0
  );
  const res = {
    ...setup,
    title: {
