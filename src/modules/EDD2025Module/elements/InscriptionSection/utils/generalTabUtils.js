...
export function buildAvanceDiario2(setup, data, data2023) {
  const arrFechas = [];
  const arrAcumulado = [];
...
export function buildAvanceDiario(data) {
  const data2024 = AVANCE_DIARIO_2024;
  const arrFechas = [];
  const arrAcumulado = [];
  const arrAcumulado2023 = [];
  const arrAcumulado2 = [];
  ...
  Object.entries(data).forEach(([key, arrays]) => {
    if (key !== "") {
      ...
      if (arrAcumulado.includes(key)) {
        const index = arrAcumulado.indexOf(key);
        arrAcumulado.splice(index, 1, arrays.porcentaje);
        arrAcumulado2023.splice(index, 1, data2024[key].porcentaje);
        arrAcumulado2.splice(index, 1, arrays.porcentaje);
      }
      ...
      if (arrTotal.includes(key)) {
        const index = arrTotal.indexOf(key);
        arrTotal.splice(index, 1, arrays.total);
        arrTotal2023.splice(index, 1, data2024[key].porcentaje);
      }
    }
  });
